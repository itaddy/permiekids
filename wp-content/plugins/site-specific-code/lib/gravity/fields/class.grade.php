 ...
 */
class LDMW_Gravity_Fields_Grade {
	const type = "aas_grades";
	...
	function add_title( $type ) {
		if ( $type == self::type )
			return __( 'AAS Membership Grades' );
	}
			...
			<select name="input_<?php echo $field['id']; ?>" id="<?php echo self::type . "-" . $field['id']; ?>" class="select gform_<?php echo self::type; ?> <?php echo esc_attr( $css ); ?> <?php echo $field['size']; ?>" <?php echo $tabindex; ?>>
			        <?php foreach ( LDMW_Users_Util::get_applicable_membership_grades() as $slug => $grade ) : ?>
				        <option value="<?php echo $slug; ?>"
				          <?php if ( $field['allowsPrepopulate'] && ! empty( $field['inputName'] ) && isset( $_GET[$field['inputName']] ) ) {
