...
if ( ! class_exists( 'IBD_Notify_Growl_Notification' ) ) :
	...
	class IBD_Notify_Growl_Notification extends IBD_Notify_Notification {
		/**
		 * @var string success|warning|error the class to be applied to the admin notification
		 */
		private $class = "";
		...
		public function __construct( $user_id, $title, $message, $args = array() ) {
			...
			add_action( 'wp_ajax_ibd_notify_growl_notify', array( $this, 'trigger_callback' ) );
			add_action( 'wp_ajax_nopriv_ibd_notify_growl_notify', array( $this, 'trigger_callback' ) );
		}
