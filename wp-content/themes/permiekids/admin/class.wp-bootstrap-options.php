 ...
 * @since 1.0
 */
class WP_BootStrap_Theme_Options {
	...
	public function __construct() {
		...
		if ( ! get_option( 'wp_bootstrap_options' ) )
	...
	public function add_pages() {
		...
		$admin_page = add_theme_page( __( 'Theme Options' ), __( 'Theme Options' ), 'manage_options', 'wp-bootstrap-options', array( &$this, 'display_page' ) );
	...
	public function create_setting( $args = array() ) {
		...
		add_settings_field( $id, $title, array( $this, 'display_setting' ), 'wp-bootstrap-options', $section, $field_args );
	}
	...
	<h2>' . __( 'Theme Options' ) . '</h2>';
		...
		settings_fields( 'wp_bootstrap_options' );
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
			...
			case 'checkbox':
				...
				echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="wp_bootstrap_options[' . $id . ']" value="1" ' . checked( $options[$id], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';
				
				break;
			
			case 'select':
				echo '<select class="select' . $field_class . '" name="wp_bootstrap_options[' . $id . ']">';
			...
			case 'radio':
				$i = 0;
				foreach ( $choices as $value => $label ) {
					echo '<input class="radio' . $field_class . '" type="radio" name="wp_bootstrap_options[' . $id . ']" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $options[$id], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
			...
			case 'textarea':
				echo '<textarea class="' . $field_class . '" id="' . $id . '" name="wp_bootstrap_options[' . $id . ']" placeholder="' . $std . '" rows="5" cols="30">' . wp_htmledit_pre( $options[$id] ) . '</textarea>';
			...
			case 'password':
				echo '<input class="regular-text' . $field_class . '" type="password" id="' . $id . '" name="wp_bootstrap_options[' . $id . ']" value="' . esc_attr( $options[$id] ) . '" />';
			...
			case 'text':
			default:
		 		echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="wp_bootstrap_options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" />';
	...
	*/
	public function register_settings() {
		...
		register_setting( 'wp_bootstrap_options', 'wp_bootstrap_options', array ( &$this, 'validate_settings' ) );
		
		foreach ( $this->sections as $slug => $title ) {
			if ( $slug == 'about' )
				add_settings_section( $slug, $title, array( &$this, 'display_about_section' ), 'wp-bootstrap-options' );
			else
				add_settings_section( $slug, $title, array( &$this, 'display_section' ), 'wp-bootstrap-options' );
		}
	...
	*/
	public function styles() {
		...
		wp_register_style( 'wp-bootstrap-admin', get_bloginfo( 'stylesheet_directory' ) . '/admin/wp-bootstrap-options.css' );
		wp_enqueue_style( 'wp-bootstrap-admin' );
...
$theme_options = new WP_BootStrap_Theme_Options();

function wp_bootstrap_option( $option ) {
	$options = get_option( 'wp_bootstrap_options' );
